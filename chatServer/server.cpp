...
Server::Server()
{
    init();
    list = new UserList();
    db = new DBMysql("127.0.0.1","root","123456","IM");
    db->db_connect();
}
...
bool Server::acceptClient()
{
    ...
    if(1 == lf.flag)//login
    {
        ...
        string sqlstr("select * from user where id_name = '");
        ...
        else
        {
            ...
            if(!strcmp(lf.id_name,row[0])&&!strcmp(lf.pwd,row[2]))
            {
                ...
                User user(row[1],lf.id_name,csockfd);//
                list->push(user);
                list->show();
    ...
    else //register
    {
        ...
        sqlstr.append(lf.id_name).append("','").append(account).append("','").append(lf.pwd).append("')");
